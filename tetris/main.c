...
int main() {
  ...
  for (;;) {
    ...
    float diff =
        (float)(now - then) / (float)SDL_GetPerformanceFrequency() * 1000.f;
...
void setup() {
  ...
  win = SDL_CreateWindow(
      "Tet", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, win_x, win_y,
  ...
  if (!ctx)
    exit(fprintf(stderr, "Could not create GL context\n"));
...
void age_garbage() {
  ...
  for (int i = 1; i < GARB_LVLS; i++)
    p->garbage[i] =
        p->garbage[i +
                   1]; // last loop looks like it goes oob, but there's room!
  p->garbage_tick = tick;
}
...
void receive_garbage() {
  ...
  int max_at_once =
      garbage_race ? 10 : 6; // no more than 6, 10 garbage at a time
...
// update everything related to current player, while the game is running
// normally
void update_player() {
  if (state != PLAY)
    return;
  ...
  if (++p->idle_time >=
      (p->grounded ? 100 : speeds[MIN(MAX_SPEED, p->level)])) {
    move(0, 1, 1);
    p->idle_time = 0;
  }
  ...
  if (p->shake_x < .01f && p->shake_x > -.01f)
    p->shake_x = .0f;
  if (p->shake_y < .01f && p->shake_y > -.01f)
    p->shake_y = .0f;
  if (p->shake_x)
    p->shake_x *= .98f;
  if (p->shake_y)
...
void bake() {
  ...
  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++) {
      ...
      if (world_i < 0 || world_i >= BWIDTH || world_j < 0 ||
          world_j >= BHEIGHT || p->row[world_j].col[world_i].color)
        tsum += tpart;
...
void update_particles() {
  for (int i = 0; i < NPARTS; i++) {
    ...
    struct player *o = q->opponent < 0 ? NULL : play + q->opponent;
    ...
    if (o && q->r &&
        normal_r < .65f + (i % 9) * .07f) // particle has an opponent target
    {
      printf("dist = %.1f\n", dist);
      ...
      float t = (dist < bs * 4) ? (dist / (bs * 6)) : .75f;
    ...
    if (o && in_rect(q->x + q->r * .5f, q->y + q->r * .5f, targ_x, targ_y - bs2,
                     bs, bs)) {
      q->r = 0.f;
      o->flash += 50;
