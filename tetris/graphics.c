...
void draw_end() {
  ...
  glUniformMatrix4fv(glGetUniformLocation(main_prog_id, "proj"), 1, GL_FALSE,
                     ortho);
...
void draw_menu() {
  ...
  set_color(0, 0, 0);
  rect(p->held.x, p->held.y + p->box_w + bs2 + line_height * (menu_pos + 1),
       p->board_w, line_height);
  draw_end();
...
void draw_mino(int x, int y, int shape, int outline, int part) {
  ...
  rect( // horizontal band
      x + (part & 8 ? 0 : bw), y + bw,
      bs - (part & 8 ? 0 : bw) - (part & 2 ? 0 : bw), bs - bw - bw);
  rect( // vertical band
      x + (part & 32 ? 0 : bw), y + (part & 1 ? 0 : bw),
...
void draw_shape(int x, int y, int color, int rot, int flags) {
  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++)
      draw_mino(x + bs * i +
                    ((flags & CENTER) ? bs2 + bs2 * center[2 * color] : 0),
                y + bs * j +
                    ((flags & CENTER) ? bs + bs2 * center[2 * color + 1] : 0),
                color, flags & OUTLINE, is_solid_part(color, rot, i, j));
}
...
void draw_player() {
  ...
  if (p->it.color) {
    ...
    set_color(8, 13, 12);
    rect(x + bs * (p->it.x + shadow.x), y + bs * top, bs * shadow.w,
  ...
  if (loss < 1.f && p->beam.color) {
    ...
    set_color(66, 74, 86);
    rect(x + bs * (p->beam.x + shadow.x) + lossw / 2, y + lossh, rw - lossw,
         rh - lossh);
  }
  ...
  if (p->flash > 8) {
    ...
    rect(x - 3 * bs2 - bs4, p->top_garb - bs4, bs + bs4 + bs4,
         garb_height * bs + bs2);
  }
  p->top_garb = y + bs * VHEIGHT;
  for (int i = 0; i < GARB_LVLS; i++)
  ...
  draw_shape(x + bs * p->it.x, y + bs * (ghost_y - 5), p->it.color, p->it.rot,
             OUTLINE);
  draw_shape(x + bs * p->it.x, y + bs * (p->it.y - 5), p->it.color, p->it.rot,
             0);
